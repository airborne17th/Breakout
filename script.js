...
images.background.src = './assets/bg-space.jpg';
...
const sounds = {
    ...
    song1: new Audio('./sounds/playlist/DiamondEyes.Everything.mp3'),
    song2: new Audio('./sounds/playlist/Disfigure.Blank.mp3'),
    song3: new Audio('./sounds/playlist/Elektronomia.SkyHigh.mp3'),
    song4: new Audio('./sounds/playlist/Janji.HeroesTonight.mp3'),
    paddle: new Audio('./sounds/paddle.mp3'), 
    music: new Audio()
}

const musicPlaylist = [sounds.song1, sounds.song2, sounds.song3, sounds.song4];
...
function initSounds() {
    iMusic = Math.floor(Math.random() * (musicPlaylist.length));
    sounds.music = new Audio(musicPlaylist[iMusic].src);
...
function detectCollision() {
    ...
    if (ball.y - ball.radius < 0) {
      ...
      if (ball.x > paddle.x && ball.x < paddle.x + paddle.width) {
        // Calculate new dx based on distance from center of ball to center of paddle
        const ballCenter = ball.x + ball.radius;
        const paddleCenter = paddle.x + paddle.width / 2;
        const distance = ballCenter - paddleCenter;
        const maxDistance = paddle.width / 2 + ball.radius;
        const ratio = distance / maxDistance;
        const angle = Math.PI / 3; // Change this value to adjust the angle of the bounce
        ball.dx = game.speed * ratio * Math.sin(angle) + 5;
        ball.dy = -game.speed * Math.cos(angle) + -5;
...
function togglePause() {
    // To Do: Pause has weird behavior sometimes to troubleshoot
    if(paused == true){
        prevSpeedx = ball.dx;
        prevSpeedy = ball.dy;
        ball.dx = 0;
